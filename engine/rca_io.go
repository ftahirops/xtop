...
func analyzeIO(db *AdaptiveThresholdDB, curr *model.Snapshot, rates *model.RateSnapshot, sp systemProfile) model.RCAEntry {
	...
	r.EvidenceV2 = append(r.EvidenceV2, emitEvidence("io.psi", model.DomainIO,
		...
		nil, nil))
	w, c = thresholdAdaptive(db, "io.dstate", 1, 10, curr)
	r.EvidenceV2 = append(r.EvidenceV2, emitEvidence("io.dstate", model.DomainIO,
		...
		nil, nil))
	w, c = thresholdAdaptive(db, "io.disk.latency", 20, 80, curr)
	...
	w, c = thresholdAdaptive(db, "io.disk.util", 70, 95, curr)
	r.EvidenceV2 = append(r.EvidenceV2, emitEvidence("io.disk.util", model.DomainIO,
		...
		nil, nil))
	w, c = thresholdAdaptive(db, "io.disk.queuedepth", 4, 16, curr)
	...
	if worstInodePct > 0 {
		w, c = thresholdAdaptive(db, "io.inode.pressure", 80, 95, curr)
	...
	appInjector.InjectIOEvidence(db, curr, &r)
